/* =============================================================
   HRA ACCOUNTANT – MAIN JAVASCRIPT (CORRECTED FOR INLINED HOME)
   ============================================================= */
(function () {
  'use strict';
  ...
  // Page content (same as original, but we may skip home injection)
  const pageContent = {
  ...
  function showPage(id, skipHash = false) {
    // CRITICAL FIX: If home page already has content (inlined HTML), skip injection to avoid unnecessary DOM replacement
    if (id === 'home') {
      const homeDiv = document.getElementById('page-home');
      if (homeDiv && homeDiv.innerHTML.trim().length > 500) {
        // Home content already present, just ensure it's active and skip injection
        stopParticles();
        ...
        requestAnimationFrame(() => {
          ...
          initParticles(); // Ensure particles start
        });
        updateTitle('home');
