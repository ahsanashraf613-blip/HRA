/* =============================================================
   HRA ACCOUNTANT – MAIN JAVASCRIPT (OPTIMIZED FOR PERFORMANCE)
   ============================================================= */
(function () {
  'use strict';
  ...
  // ---------- Particles (deferred initialization) ----------
  ...
  // ---------- HEAVY CONTENT (loaded lazily after page interactive) ----------
  let serviceData = null;
  let pageContent = null;

  // Function to load heavy data (simulated here as inline, but defer execution)
  function loadHeavyContent() {
    if (serviceData && pageContent) return;
    // All original content strings are here – but they are not executed until after load
    serviceData = {
  ...
  function showPage(id, skipHash = false) {
    if (!pageContent) loadHeavyContent(); // ensure content loaded
    stopParticles();
    ...
    requestAnimationFrame(() => {
      ...
      if (id === 'home') {
        // Defer particle initialization even more
  ...
  function showServicePage(key) {
    if (!serviceData) loadHeavyContent();
  ...
  // All form handlers and helpers (unchanged, but included for brevity)
  ...
  function handleHashChange() {
    if (!pageContent) loadHeavyContent(); // ensure content loaded before routing
  ...
  // Defer full initialization until after load
