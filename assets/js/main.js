...
(function () {
  ...
  // Page content for non-home pages (about, contact, register, privacy)
  const pageContent = {
  ...
  // CORRECTED showPage: skips injection if home already has content
  function showPage(id, skipHash = false) {
    ...
    if (id === 'home') {
      ...
      if (homeDiv && homeDiv.innerHTML.trim().length > 500) {
        // Home content already present; just ensure active state and animations
    ...
    // For other pages or if home not pre-filled, use cache or pageContent
    stopParticles();
