...
(function () {
  ...
  // Loading Screen Handler - Hide when page is fully loaded
  function initLoadingScreen() {
    const loadingScreen = document.getElementById('loading-screen');
    if (!loadingScreen) return;
    
    // Fallback: hide after max 3 seconds in case something hangs
    ...
    // Hide when window is fully loaded (all images, scripts, etc.)
    ...
    if (document.readyState === 'complete') {
      clearTimeout(fallbackTimeout);
      setTimeout(() => loadingScreen.classList.add('hide'), 100);
    }
  }
