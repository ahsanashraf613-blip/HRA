...
(function () {
  ...
  // ---------- PARTICLES AND 3D TILT (unchanged) ----------
  ...
  // Page content (same as original – full content)
  const pageContent = {
  ...
  // ---------- MODIFIED showPage function: skip injection if page already has content ----------
  function showPage(id, skipHash = false) {
    stopParticles();
    const page = document.getElementById('page-' + id);
    if (!page) return;
    
    // For service page, handle specially
    if (id === 'service') {
      // Ensure other pages are inactive
    ...
    // For other pages: only inject if the page is empty (no child elements)
    // This prevents overwriting already-inlined content (like home page)
    if (!pageCache[id] && pageContent[id]) pageCache[id] = pageContent[id];
    if (pageCache[id] && page.children.length === 0) {
      page.innerHTML = pageCache[id];
    }
